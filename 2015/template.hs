...
getLocals
  = filter (\(_, (scp, _)) -> scp == Local)

getGlobals :: State -> State
getGlobals
  = filter (\(_, (scp, _)) -> scp == Global)
...
applyOp Mul (I c) (I c')
  = I (c * c')
applyOp Less (I c) (I c')
  = I (fromEnum (c < c'))
applyOp Equal (I c) (I c')
  = I (fromEnum (c == c'))
applyOp Index (A ics) (I i)
  = case lookup i ics of
      Just c  -> I c
      Nothing -> I 0
...
eval (Var id) _ s
  = getValue id s
eval (Cond p q r) defs s
  = case eval p defs s of
      I 1 -> eval q defs s
      I 0 -> eval r defs  s
...
executeStatement (Assign id e) fDefs _ s
  = updateVar (id, eval e fDefs s) s
executeStatement (AssignA id e e') fDefs _ s
  = updateVar (id, assignArray arr i c) s
  where
    i = eval e fDefs s
    c = eval e' fDefs s
    arr = getValue id s
executeStatement (If p b b') fDefs pDefs s
  = case eval p fDefs s of
      I 1 -> executeBlock b fDefs pDefs s
      I 0 -> executeBlock b' fDefs pDefs s
executeStatement (While p b) fDefs pDefs s
  = case eval p fDefs s of
      I 1 -> executeStatement (While p b) fDefs pDefs s'
      I 0 -> s
  where
    s' = executeBlock b fDefs pDefs s
executeStatement (Call vId pId es) fDefs pDefs s
  | null vId  = finalState
  | otherwise = updateVar (vId, getValue "$res" s'') finalState
  where 
    (as, b) = lookUp pId pDefs
    vs = evalArgs es fDefs s
    s' = bindArgs as vs ++ getGlobals s
    s'' = executeBlock b fDefs pDefs s'
    finalState = getLocals s ++ getGlobals s''
executeStatement (Return e) fDefs pDefs s
  = ("$res", (Local, eval e fDefs s)) : s


executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock b fDefs pDefs s
  = foldl (\s smt -> executeStatement smt fDefs pDefs s) s b
...
translate' e [] ids
  = ([], e, ids)
translate' (Cond p q r) mTable (id : ids)
  = ([If p (b ++ [Assign id res]) (b' ++ [Assign id res'])], Var id, ids'')
  where
    (b, res, ids')    = translate' q mTable ids
    (b', res', ids'') = translate' r mTable ids'
translate' e@(FunApp f es) mTable (id : ids)
  = case lookup f mTable of
      Just mf -> ([Call id mf es], Var id, ids)
      Nothing -> ([], e, id : ids)
translate' (OpApp op e e') mTable ids
  = (b ++ b', OpApp op res res', ids'')
  where
    (b, res, ids')  = translate' e mTable ids
    (b', res', ids'') = translate' e' mTable ids'
translate' e _ ids
  = ([], e, ids)
