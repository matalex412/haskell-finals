...
getChild n xml
  ...
  | otherwise = head cs
  where
    cs = getChildren n xml

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild xml (Element n as xmls)
  = Element n as (xmls ++ [xml])
...
parse' ('<' : xs) s
  = parse' xs'' (Element name as [] : s)
  where
    (name, xs') = parseName xs
    (as, xs'') = parseAttributes xs'
parse' xs s
  = parse' xs' (addText text s)
  where
    (text, xs')= span (/= '<') xs
...
getAtPath p@(x : _) ctx 
  | x == '.', null rest  = [ ctx ]
  | x == '.'             = getAtPath (tail rest) ctx
  | x == '@'             = [ Text (getAttribute (tail step) ctx) ]
  | null rest            = children
  | isAlpha x            = concatMap (getAtPath (tail rest)) children
  where
    (step, rest) = span (/= '/') p
    children     = getChildren step ctx
