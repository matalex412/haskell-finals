...
foldConst (Phi (Const c) (Const c')) | c == c'
  = Const c
...
sub :: Id -> Int -> Exp -> Exp
-- Pre: the expression is in SSA form
sub id v (Var id') | id == id'
  = Const v
sub id v (Apply op e e')
  = foldConst (Apply op (sub id v e) (sub id v e'))
sub id v (Phi e e')
  = foldConst (Phi (sub id v e) (sub id v e'))
sub _ _ e
  = e
...
repeatScan :: (Exp -> Exp) -> Block -> Block
repeatScan allSub b
  | b == b'   = b
  | otherwise = repeatScan allSub' b'
  where
    (allSub', b') = scan allSub b

scan :: (Exp -> Exp) -> Block -> (Exp -> Exp, Block)
scan _ []
  = (id, [])
scan allSub (smt : smts)
  = case smt of
      Assign "$return" e 
        ->  (allSub', Assign "$return" (allSub e) : b')
      Assign id e 
        ->  case allSub e of
              Const c -> (sub id c . allSub', b')
              e'      -> (allSub', Assign id e' : b')
      If p q r
        -> let (qSub, q') = scan allSub q; (rSub, r') = scan allSub r
           in (qSub . rSub . allSub', If (allSub p) q' r' : b')
      DoWhile q e
        -> let (qSub, q') = scan allSub q
           in (qSub . allSub', DoWhile q' (allSub e) : b')
  where
    (allSub', b') = scan allSub smts
